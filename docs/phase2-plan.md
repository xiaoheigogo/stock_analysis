...
> 创建日期：2026-04-29 | 最后更新：2026-05-12 | 状态：设计中，Phase 1 收尾后实施

## 0. 前置条件

Phase 1 基本完成：
- 4 种模型训练完成（LSTM 最佳，val_loss=0.000129）
- 预测管线可用，API + 前端展示真实预测结果
- 数据管线完整（获取 → 清洗 → 训练 → 预测）

**Phase 2 启动前需完成的 Phase 1 收尾项**：

| 优先级 | 条目 | 预估耗时 |
|--------|------|---------|
| P0 | 修复模型加载路径（切换为 LSTM 最佳模型） | 10 分钟 |
| P0 | 移除 `batch_predict()` 硬编码 10 支限制 | 5 分钟 |
| P1 | 修复 `main.py server` 命令 | 15 分钟 |
| P1 | 实现 `/api/stock/autocomplete` 端点 | 30 分钟 |
| P2 | 实现置信度/风险评分真实计算 | 2 小时 |
| P2 | 解除 `save_recommendations()` DB 写入 | 30 分钟 |
| P2 | 创建 `recommendations` 表 | 15 分钟 |
| P3 | 实现 `evaluate_recommendations()` | 3 小时 |
| P3 | 前端"历史预测"视图 | 4 小时 |
| P3 | `auto_trainer.py` 接入真实训练管线 | 4 小时 |
...
Phase 2 完成后进入 **Phase 3：多维分析增强**（详见 `docs/phase3-plan.md`）：
